package net.waymire.tyranny.authserver.boot;
...
public final class Boot 
{
	...
	private static final String AUTHSERVER_ENVIRONMENT_CLASSNAME = "net.waymire.tyranny.authserver.AuthserverEnvironment";
	private static final String AUTHSERVER_MASTER_CLASSNAME = "net.waymire.tyranny.authserver.AuthserverMaster";
	private static final String AUTHSERVER_MASTER_METHODNAME = "main";
	...
	private void initBootClassLoader()
	{
		try
		{
			...
			Object environment = loadAuthserverEnvironment(systemClassLoader);
	...
	private Object loadAuthserverEnvironment(ClassLoader loader)
	{
		Object ret = null;
		
		try
		{
			Class<?> c = loader.loadClass(AUTHSERVER_ENVIRONMENT_CLASSNAME);
	...
	private void initAuthserverEnvironment()
	{
		try
		{
			Object environment = loadAuthserverEnvironment(Thread.currentThread().getContextClassLoader());
			...
			Class<?> envClass = Thread.currentThread().getContextClassLoader().loadClass(AUTHSERVER_ENVIRONMENT_CLASSNAME);
			...
			appRegistryRegisterMethod.invoke(appRegistryInstance, envClass, environment);
	...
	private void launchMaster()
	{
		try
		{
			Class<?> masterClass = Thread.currentThread().getContextClassLoader().loadClass(AUTHSERVER_MASTER_CLASSNAME);
			Method main = masterClass.getMethod(AUTHSERVER_MASTER_METHODNAME,new Class<?>[] { (new String[0]).getClass() });
